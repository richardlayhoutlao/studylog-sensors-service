...
from score import compute_study_score
...
def _load_io_module(module_name):
    module_path = os.path.join(os.path.dirname(__file__), "io", f"{module_name}.py")
...
def _evaluate_discomfort(temp_c, light_pct, sound_pct, counters):
    ...
    if cold_n >= NEED_CONSEC:
        reasons.append("FROID")
    if hot_n >= NEED_CONSEC:
        reasons.append("CHAUD")
...
def loop(led, button, lcd_ui, sound_processor):
    counters = (0, 0, 0, 0, 0)
    ...
    while True:
        ...
        reasons, counters = _evaluate_discomfort(temp_c, light_pct, sound_pct, counters)
        ...
        if button.show_score_mode:
            ...
            lcd_ui.show_stats(temp_str, light_pct, sound_pct, uncomfortable, reasons)
...
if __name__ == "__main__":
    ...
    try:
        ...
        loop(led, button, lcd_ui, sound_processor)
